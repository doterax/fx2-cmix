...
inline Rwkv::Rwkv(unsigned int input_size, unsigned int output_size, unsigned int
...
inline void Rwkv::SetInput(const Eigen::VectorXf& input) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
    layer_input_[epoch_][i].head(input_size_) = input.head(input_size_);
  }
}

inline Eigen::VectorXf& Rwkv::Perceive(unsigned int input) {
  ...
  if (epoch_ == 0 && ++bptt_cycle_ >= bptt_period_) {
    ...
    for (int epoch = horizon_ - 1; epoch >= bptt_start; --epoch) {
      for (int layer = layers_.size() - 1; layer >= 0; --layer) {
        ...
        hidden_error_.noalias() += output_layer_[epoch].block(offset, 0,
            num_cells_, output_size_) * errors_vec;
...
inline Eigen::VectorXf& Rwkv::Predict(unsigned int input) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
