...
inline XLstm::XLstm(unsigned int input_size, unsigned int output_size,
    unsigned int num_cells, unsigned int num_layers, int horizon,
    float learning_rate, float gradient_clip, int bptt_depth,
    int bptt_period) : input_history_(horizon),
  ...
  for (int epoch = 0; epoch < horizon; ++epoch) {
    for (unsigned int i = 0; i < num_layers; ++i) {
      layer_input_[epoch][i] =
          Eigen::VectorXf::Zero(input_size + 1 + num_cells * 2);
      layer_input_[epoch][i][layer_input_[epoch][i].size() - 1] = 1;
    }
  }

  for (unsigned int i = 0; i < num_layers; ++i) {
    layers_.emplace_back(layer_input_[0][i].size() + output_size,
        input_size_, output_size_, num_cells, horizon, gradient_clip,
        learning_rate);
  }
}
...
inline void XLstm::SetInput(const Eigen::VectorXf& input) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
    layer_input_[epoch_][i].head(input_size_) = input.head(input_size_);
  }
}

inline Eigen::VectorXf& XLstm::Perceive(unsigned int input) {
...
inline Eigen::VectorXf& XLstm::Predict(unsigned int input) {
  for (unsigned int i = 0; i < layers_.size(); ++i) {
